...
export async function GET() {
  // Use service_role key to bypass RLS for seeding, or just anon key if RLS allows it.
  // We'll use the service_role key to guarantee insertion bypasses RLS.
  ...
  console.log("Seeding databases via API...");
  let errors: string[] = [];
  ...
  for (const v of vehicles) {
    ...
    if (error && error.code !== '23505') errors.push(`Vehicle error: ${error.message}`);
  }
  ...
  for (const d of drivers) {
    ...
    if (error && error.code !== '23505') errors.push(`Driver error: ${error.message}`);
  }
  ...
  if (vData && dData) {
    ...
    if (van && trk1 && alex && sarah) {
      ...
      for (const t of trips) {
        ...
        if (error) errors.push(`Trip error: ${error.message}`);
      }
      ...
      if (trk2) {
         ...
         for (const l of logs) {
            const { error: logErr } = await supabase.from('maintenance_logs').insert([l]);
            if(logErr) errors.push(`Maintenance log error: ${logErr.message}`)
         }
      }
      ...
      if (tData && tData.length > 0) {
        ...
        for (const e of expenses) {
          const { error: expErr } = await supabase.from('expenses').insert([e]);
          if(expErr) errors.push(`Expense error: ${expErr.message}`)
        }
      }
    }
  }

  return NextResponse.json({ success: true, message: "Seeding complete!", errors: errors.length > 0 ? errors : undefined });
}
